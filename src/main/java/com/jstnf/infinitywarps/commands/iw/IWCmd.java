...
public class IWCmd extends IWExecutor implements CommandExecutor
{
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args)
	{
		...
		else
		{
			...
			switch (sub.toLowerCase())
			{
				...
				case "help":
					/* Permission */
					if (sender.hasPermission(CommandEntry.IW_HELP.permission()))
					{
						int page = 1;
						if (args.length > 1)
						{
					...
					{
						sender.sendMessage(plugin.configManager.getMessage(IWLocale.NO_PERMISSION_HELP, true));
					}
					break;
				default:
	...
	private void sendCommandHelp(CommandSender sender, int page)
	{
		...
		CommandEntry[] entries = IWUtils.generateEntries(sender);
		...
		// In order to use the help command, you actually need the "infinitywarps.help" permission.
		// So it is impossible to have 0 entries here.
		...
		/* We should have a check just in case to avoid any unhandled exceptions. */
		if (pages == 0)
		{
			/* If we somehow got here, there's a weird problem! */
			sender.sendMessage("Somehow, we found 0 help pages... this shouldn't happen. Report this to the plugin developer!");
			return;
		}
